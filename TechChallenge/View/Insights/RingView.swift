...
struct RingView: View {
    ...
    var body: some View {
        ZStack {
            ForEach(Category.allCases[1...5].indices) { categoryIndex in
                PartialCircleShape(
                    offset: offset(for: categoryIndex),
                    ratio: ratio(for: categoryIndex)
                )
...
struct RingView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        VStack {
            ...
            RingView(transactions: ModelData.shared.sampleTransactions)
                .scaledToFit()
        }
        .padding()

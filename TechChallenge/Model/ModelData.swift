...
final class ModelData: ObservableObject {
    ...
    @Published var sampleTransactions: [TransactionModel] = [
        TransactionModel(
            id: 1,
            name: "Movie Night",
    ...
    func graphData() -> [Data] {
        var data: [Data] = []
        let transactions = self.sampleTransactions.pinnedTransactions()
        let sumOfAllTransaction = transactions.sumOfTransactions()
        var currentAngle: Double = 0
        for (i, category) in TransactionModel.Category.allCases[1...5].enumerated() {
            let sumOfTransaction = transactions.filterTransactions(by: category).sumOfTransactions()
            let angle = sumOfTransaction/sumOfAllTransaction
            let ratio = sumOfTransaction/sumOfAllTransaction
            data.append(Data(category: i+1,
                             offset: currentAngle,
                             ratio: ratio))
            currentAngle += angle
        }
        return data.sorted(by: { $0.offset < $1.offset })
    }
}
...
class ModelDataContainer: ObservableObject {
